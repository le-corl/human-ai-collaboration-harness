...
7. Ensure each role memory has collaboration calibration fields. Add the default `50%`, zero samples, and current executor when the fields are absent; never reset an existing score. Preserve or restore the matching role-local file in `collaboration/feedback/` without reading other feedback history.
8. Read only the selected profile in `collaboration/profiles/`, choose the role needed for the user's original request, and continue that request instead of ending after setup.
9. Do not expand product scope or perform commits, pushes, publication, account changes, or other external actions unless the user has authorized them.
